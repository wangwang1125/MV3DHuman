...
class AdaptiveTokenMerging(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Handle CLS token preservation
        preserve_cls = self.token_merger.preserve_cls
        if preserve_cls and N > 1:
            # 如果有CLS token，target_tokens需要包含CLS token
        ...
        r = max(self.min_r, min(self.max_r, N // effective_target))

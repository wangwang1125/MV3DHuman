#!/bin/bash
# 启动多视图RGB重建 Gradio 界面（使用 Hunyuan3D-2mv 预训练模型）
# 仅四视图彩色图像，不使用法线图
...
# 4. 如果还没找到，尝试查找原始版本的checkpoint（向后兼容）
if [ -z "$RGB_CHECKPOINT_PATH" ]; then
...
if [ -z "$RGB_CHECKPOINT_PATH" ] || [ ! -f "$RGB_CHECKPOINT_PATH" ]; then
    echo "⚠️  警告: 未找到多视图RGB checkpoint，将使用基础 Hunyuan3D-2mv 模型"
...
echo ""
echo "================================================"
echo "启动多视图RGB重建 Gradio 界面 (Hunyuan3D-2mv)"
echo "================================================"
echo "模型: $MODEL_PATH / $SUBFOLDER"
echo "端口: $PORT"
echo "地址: http://$HOST:$PORT"
echo "模式: 仅四视图RGB重建（不使用法线图）"
if [ -n "$RGB_CHECKPOINT_PATH" ]; then
    echo "Checkpoint: $RGB_CHECKPOINT_PATH"
else
    echo "Checkpoint: 使用基础模型（未加载训练权重）"
fi
echo "================================================"
echo ""
